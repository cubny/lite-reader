...
//go:generate mockgen -destination=./infra/http/api/auth_mock.go -package=mocks -mock_names=AuthService=AuthService github.com/cubny/lite-reader/internal/infra/http/api AuthService
//go:generate mockgen -destination=./infra/http/api/folder_mock.go -package=mocks -mock_names=FolderService=FolderService github.com/cubny/lite-reader/internal/infra/http/api FolderService

//go:generate mockgen -destination=./infra/job/dependencies_mock.go -package=mocks -mock_names=FeedService=FeedService,ItemService=ItemService,UserService=UserService github.com/cubny/lite-reader/internal/infra/job FeedService,ItemService,UserService
